...
public class ImportIntroSkipperDataTask : IScheduledTask
{
    ...
    public async Task ExecuteAsync(IProgress<double> progress, CancellationToken cancellationToken)
    {
        ...
        var introSkipperDbPath = Path.Combine(_applicationPaths.DataPath, "introskipper", "introskipper.db");
        ...
        for (int i = 0; i < itemIds.Count; i++)
        {
            ...
            foreach (var providerName in _allProviderNames)
            {
                if (!existingStatuses.Contains((itemId, providerName)))
                {
    ...
    private string ComputeImportConfigHash(string pluginConfigPath)
    {
        ...
        {
            var path = Path.Combine(pluginConfigPath, candidate);
            if (File.Exists(path))
            {
                try
                {
